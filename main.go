...
func defence(charCame, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", charCame, 10+randint(5, 10))
	}
	if charClass == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", charCame, 10+randint(-2, 2))
	}
	if charClass == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", charCame, 10+randint(2, 5))
	}
	return "неизвестный класс персонажа"
}
...
func startTraining(charCame, charClass string) string {
	if charClass == "warrior" {
	...
	for cmd != "skip" {
		...
		if cmd == "attack" {
			fmt.Println(attack(charCame, charClass))
		}

		if cmd == "defence" {
			fmt.Println(defence(charCame, charClass))
		}

		if cmd == "special" {
			fmt.Println(special(charCame, charClass))
		}
	}
...
func main() {
	...
	fmt.Println(startTraining(charCame, charClass))
}
